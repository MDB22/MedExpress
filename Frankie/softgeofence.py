...
TODO: 	Take into acount altitude
		Create property decorators for necessary classes (stop bad values from being set)
		Develop a better way to change the original geofence color
...
class Geofence(object):
	...
	def __init__(self, coords):
		...
		if Geofence.__first_instantiation:
			Geofence.__AbsoluteCoord = self.coords[0]
			Geofence.__first_instantiation = False
	...
	def generate_line_representation(self):
		...
		prev_coord = Coordinate.generate_coord_from_geo(Geofence.__AbsoluteCoord, self.coords[0])
		...
		for i in range(0, self.num_coords - 1):
			...
			if (i != self.num_coords - 2):
				...
				next_coord = Coordinate.generate_coord_from_geo(Geofence.__AbsoluteCoord, self.coords[0])
	...
	def is_coord_inside(self, location= None, lat = None, lon = None):
		"""
		Pass in a coordinate to see if it is inside the geofence (calling object)

		Note: Over a 12km range this method is accurate to within approximately 2-3m (Could be higher in fringe cases)

		:param location: A :py:class:`LocationGlobal` object that represents our point of interest
		...
		if location is None:
			if ((lat is None) | (lon is None)):
				sys.exit("Need lat and lon to be not None")
			
			location = LocationGlobal(lat, lon)
		...
		bearing = Geofence.__AbsoluteCoord.bearing_between(location)

		# Generate a coordinate for our point of interest (poi)
		poi = Coordinate.generate_coord_from_geo(Geofence.__AbsoluteCoord, location)
