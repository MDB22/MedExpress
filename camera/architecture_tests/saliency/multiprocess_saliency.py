...
class MPSaliency:
    ...
    def __init__(self, img, use_numpy_fft=True, gauss_kernel=(5, 5)):
        ...
        # needed for the multiprocessing, register some of the methods
        # so that they can be unpickled

        copy_reg.pickle(types.MethodType, self._pickle_method(self._get_channel_sal_magn), self._unpickle_method(self._get_channel_sal_magn))
        ...
        if self.need_saliency_map:
            ...
            if len(self.frame_orig.shape) == 2:
                ...
                #for c in xrange(self.frame_small.shape[2]):
                    ...
                    #tasks.append(self._get_channel_sal_magn(small, c)

                #for c in xrange(self.frame_small.shape[2]):
                for i in pool.imap_unordered(self._get_channel_sal_magn, range(3)):
                    pass
                
                #results = [pool.apply_async(self._get_channel_sal_magn(self.frame_small[:, :, c], c ) for c in xrange(self.frame_small.shape[2])]
                ...
                for c in xrange(self.frame_small.shape[2]):
                    small = self.frame_small[:, :, c]
                    sal[:, :, c] = self._get_channel_sal_magn(small, c)
    ...
    def _get_channel_sal_magn(self, channel, channel_no):
